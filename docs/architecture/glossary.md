...
| **Turn Interpreter** | Port that turns free text into a Turn Interpretation. It is a *language* component and nothing else: it never decides what happens next, never touches the Trip Plan, and resolves relative dates against the Clock before any value reaches the domain. First adapter is keyword-based and deterministic; later adapters call the LLM Gateway. Everything it may know is a **Dialogue Context**. |
...
| **Resting State** | The subset of Dialogue States a session is ever *observed* in between two turns — `GREETING`, `ELICITING_BLOCKING`, `AWAITING_CHOICE`, `OFFERING_UNMENTIONED`, `CLOSED`. Everything else is passed through inside one turn. A turn that changes nothing returns the session to the Resting State it arrived in. |
...
| **Pending Question** | The blocking question currently outstanding: the Blocking Rule it is about, every field that would help satisfy it, the turn it was asked on, and how many times it has been asked. It names a **rule** rather than a field, because an obligation may be satisfied in more than one way — and the count is what lets the Director stop asking. |
