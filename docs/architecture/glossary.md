...
| **Diagnostic** | An opaque code on an Option Source Result or an Option Slate saying something about *how* the answer was produced — `synthesised`, `no_match`, `source_failed:<id>`, `source_timed_out:<id>` (a source abandoned at its per-source deadline, or one that reported having overrun it), `filtered_out` and `nothing_found` (every source answered, none of them failed, and between them they had nothing). Never a sentence, and opaque like an Agenda Reason Code: a consumer that does not recognise one records it rather than acting on it. |
