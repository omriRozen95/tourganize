...
CLOCKS: list[tuple[str, Callable[[], Clock]]] = [
...
@pytest.mark.parametrize(("name", "build"), CLOCKS, ids=[name for name, _ in CLOCKS])
def test_now_returns_an_aware_datetime(name: str, build: Callable[[], Clock]) -> None:
    moment = build().now()
...
@pytest.mark.parametrize(("name", "build"), CLOCKS, ids=[name for name, _ in CLOCKS])
def test_time_never_runs_backwards(name: str, build: Callable[[], Clock]) -> None:
    clock = build()
...
@pytest.mark.parametrize(("name", "build"), CLOCKS, ids=[name for name, _ in CLOCKS])
def test_the_port_is_satisfied_structurally(name: str, build: Callable[[], Clock]) -> None:
